...
export async function POST(request:Request){
  try{
    ...
    const rows=await sql<(AppUser&{passwordHash:string|null;failedLoginAttempts:number;lockedUntil:Date|null})[]>`select id,email,coalesce(display_name,email) as "displayName",role,status,password_hash as "passwordHash",failed_login_attempts as "failedLoginAttempts",locked_until as "lockedUntil" from users where email=${parsed.data.email} limit 1`;
